...
def migration_current(engine, url, db_name, verbose=False):
    with engine.begin() as connection:
        cfg.attributes["connection"] = connection
        logger.info(f"get {db_name} database current version")
        command.current(cfg, verbose=verbose)


def migration_upgrade(engine, url, db_name, revision, sql, tag, force):
    with engine.begin() as connection:
        cfg.attributes["connection"] = connection
        if force:
...
def migration_downgrade(engine, url, db_name, revision, sql, tag):
    with engine.begin() as connection:
        cfg.attributes["connection"] = connection
...
def migration_revision(engine, url, db_name, message, autogenerate, **kwargs):
    with engine.begin() as connection:
        cfg.attributes["connection"] = connection
        logger.info(f"autogenerate {db_name} database schema")
        command.revision(cfg, autogenerate=autogenerate, message=message, **kwargs)
