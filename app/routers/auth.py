...
def login(user_cridentials: OAuth2PasswordRequestForm = Depends() ,db: Connection = Depends(get_db)):

    check_email_stmt = select(users_table).where(users_table.c.email == user_cridentials.username)
