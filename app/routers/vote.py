...
from routers.validation.fast_api_pydantic.vote import Vote
...
def vote(vote: Vote, current_user_data: CursorResult= Depends(oauth2.get_current_user), db: Connection = Depends(get_db)):
    current_user_id = current_user_data[0]

    find_vote_stmt = select(votes_table).where(
        votes_table.c.user_id == current_user_id,
        votes_table.c.post_id == vote.post_id
        )
    ...
    if vote.dir == 1:
        ...
        check_post_stmt = select(posts_table).where(
            posts_table.c.id == vote.post_id
        )
